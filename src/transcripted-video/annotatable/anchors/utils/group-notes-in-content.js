...
function findAnchorByRange(anchors, range) {
	...
	return anchors.find(anchor => {
		...
		return (
			rangeStart.isFloatLessThanOrEqual(startTime) &&
			rangeEnd.isFloatGreaterThanOrEqual(endTime)
		);
	});
}
...
export default function groupNotesInContent(content, container, notes) {
	...
	const groups = notes.reduce((acc, note) => {
		...
		const anchor =
			(range && findAnchorByRange(anchors, range)) ||
			findAnchorById(anchors, ContainerId);
