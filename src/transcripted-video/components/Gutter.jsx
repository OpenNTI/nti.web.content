...
export default class Gutter extends React.Component {
	...
	getYForNote = note => {
		...
		try {
			...
			const cue = containerId
				? (cues || []).find(c => c.getID && c.getID() === containerId)
				: null;
	 ...
	 *
	 * @param {object} param
	 * @param {object} param.applicableRange
	 * @param {object} param.applicableRange.start
	 * @param {number} param.applicableRange.start.seconds
	 * @param {object} param.applicableRange.end
	 * @param {number} param.applicableRange.end.seconds
	 * @see getRangeFor
	 * @see getRangeFromCue
	...
	getRangeFromCue = ({ ContainerId: containerId }) => {
		...
		const { startTime: start, endTime: end } =
			(cues || []).find(
				cue => cue.getID && cue.getID() === containerId
			) || {};
	...
	getRangeFor = note => {
		const { start, end } =
			this.getRangeFromNote(note) || this.getRangeFromCue(note);
		return { start, end };
	};

	renderNoteGroups() {
		const { notes, notesFilter, yForCue, yForTime } = this.props;
		...
		const sort = ([topA], [topB]) =>
			parseInt(topA, 10) - parseInt(topB, 10);
	...
	renderActionWidget() {
		const {
			yForTime,
			actionInfo: { time: { start } = {}, top: actionTop } = {},
		} = this.props;
		...
		try {
			const top =
				(actionTop != null ? actionTop : yForTime(start)) + 'px';
			return (
				<ActionWidget
					style={{ top }}
					actionInfo={this.props.actionInfo}
					yForTime={yForTime}
				/>
			);
		} catch (e) {
