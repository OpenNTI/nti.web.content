...
const Hit = ({ hit }) => {
	...
	return (
		<LinkTo.Object
			...
			data-target-mimetype={targetMimeType}
		>
			<div className={cx('hit-title')}>
				{title ? <Fragment fragment={title[0]} /> : containerTitle}
			</div>
			<Fragments fragments={content} className={cx('content-hits')} />
			<Fragments
				fragments={keywords}
				className={cx('keyword-hits')}
				tag="div"
			/>
		</LinkTo.Object>
	);
...
const Fragments = ({ fragments, className, ...other }) =>
	!fragments || !fragments.length ? null : (
		<div className={cx('hit-fragments', className)}>
			{fragments.map((f, i) => (
				<Fragment key={i} fragment={f} {...other} />
			))}
		</div>
	);
...
const Fragment = ({ fragment: { Matches: matches }, tag: Tag = 'p' }) =>
	matches.map((m, i) => (
		<Tag key={i} className={cx('fragment')} {...rawContent(m)} />
	));
