...
export default class IframeEditor extends React.Component {
	...
	blankComponent = () => {
		return (
			<div className="iframe-editor-blank">
	...
	innerComponent = () => {
		...
		if (iframeObj) {
			...
			iframe['height'] =
				iframe['height'] === '' ? '100%' : iframe['height'];
		}
		...
		return hasData ? (
			<div
				className="editor-iframe-embed"
				{...rawContent(iframe.outerHTML)}
			/>
		) : (
			this.blankComponent()
		);
	...
	render() {
		return <div className="iframe-editor">{this.innerComponent()}</div>;
	}
}
