...
const DEFAULT_TEXT = {
	publish: {
		...
		message:
			'Make sure to add your reading to a lesson so your students can access it.',
		delete: 'Delete',
		save: 'Publish',
	},
	publishChanges: {
		...
		message:
			'Publishing will replace the version of this reading that is visible to students.',
...
export default class ContentEditorPublish extends React.Component {
	...
	onStoreChange = data => {
		...
		if (type === PUBLISHING) {
			...
			type === SET_ERROR &&
			(NTIID === contentPackage.NTIID || NTIID === contentPackage.OID)
		) {
			this.onPublishError();
		}
	};
	...
	onRenderJobChanged() {
		this.setState(
			...
			() => {
				if (
					this.state.renderJob &&
					!this.state.renderJob.isPending &&
					!this.state.error
				) {
	...
	onPublishError() {
		const { contentPackage } = this.props;
		const error =
			Store.getErrorFor(contentPackage.NTIID, 'publish') ||
			Store.getErrorFor(contentPackage.OID, 'publish');
		...
		return (
			...
			>
				<Flyout.Triggered
					...
					trigger={trigger}
					className="content-editor-publish"
					arrow
	...
	renderPublished(error) {
		return (
			<div className="content-editor-publish-menu published">
				...
				<div className="action" onClick={this.onUnpublish}>
					{t('publishChanges.unpublish')}
				</div>
				<div className="delete" onClick={this.onDelete}>
					<i className="icon-delete" />
